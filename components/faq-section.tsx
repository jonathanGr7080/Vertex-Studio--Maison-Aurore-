          ...
          <div className="lg:col-span-7 lg:col-start-6">
            {faqItems.map((item) => (
              <details key={item.question} className="group border-b border-nuit/15 first:border-t">
                <summary className="flex min-h-14 cursor-pointer list-none items-center justify-between gap-6 py-4 font-display text-xl font-medium marker:content-none [&::-webkit-details-marker]:hidden">
                  {item.question}
                  <span
                    aria-hidden
                    className="text-2xl font-light text-laiton-fonce transition-transform duration-200 group-open:rotate-45"
                  >
                    +
                  </span>
                </summary>
